$(document).ready(function() {
  ...
  $("#choose-team-submit").on("click", function(event) {
    ...
    let newUser = {
      name_first: userFirstNameInput.val().trim(),
      user_last: userLastNameInput.val().trim(),
    ...
    $.ajax({
      method: "GET",
      url: "https://pokeapi.co/api/v2/pokemon/" + mons.name
    }).then(function(res) {
      let pokemonInfo1 = {
        move1: {
    ...
    $.ajax("/api/user/team", {
      ...
      localStorage.token = res.meta.token;
      location.reload();
    });
  });
